...
class ThumbnailGenerator:
    ...
    def generate_thumbnail(self, title, style="cinematic_documentary"):
        ...
        try:
            backdrop = ImageClip(backdrop_file).set_duration(1)
            w, h = backdrop.size
            
            # Add Dark Gradient Overlay for text readability
            from moviepy.editor import ColorClip
            gradient = ColorClip(size=(w, h//2), color=(0,0,0)).set_opacity(0.6).set_position(('center', 'bottom'))
            ...
            badge_bg = ColorClip(size=(400, 60), color=(197, 160, 89)).set_position((50, 50)) # Gold bar
            badge_txt = TextClip(
                badge_text,
                fontsize=30,
                color='black',
